...
ringer is **not a member of the root Cargo workspace**. It has its own
...
`nix-build -A ringer-desc` runs `protoc --include_imports` over the single file
...
```sh
RINGER_API_KEY=$(cat ~/.config/ringer/api-key) \
  ringer --origin "Grenoble, France" \
         --destination "Lyon, France" \
         --log-dir /tmp/ringerlog
```
...
```sh
ringer --dump-descriptor /tmp/ringer.desc
mkdir -p /tmp/ringer-db
reproto --schema-db-out /tmp/ringer-db/ringer.desc /tmp/ringer.desc
protolens /tmp/ringerlog/log.pb --descriptor-set /tmp/ringer-db/ringer.desc
```
...
protolens /tmp/query.pb --descriptor-set /tmp/ringer-db/ringer.desc
```
