...
# 0356 — script trigger predicates
...
The fix is to let a step declare a **trigger**: a predicate over the session
state. After every user action, protolens evaluates the predicate; when it
becomes true, the step advances automatically. The presenter performs the
action the text invited, and the script responds — no `space` needed.

## Goals

- **G1.** A step may declare a `trigger:` list of predicate items. When
  all items hold simultaneously, the step advances to the next step
  automatically.
- **G2.** `space` and `Backspace` remain unconditional: they always
  advance/retreat regardless of whether a trigger is defined.
- **G3.** The trigger is evaluated immediately after `script_apply` (i.e.
...
- **G4.** Six predicate keys: `visible`, `folded`, `wire`, `type`,
  `caret`, and `not`. Each is a flat single-key mapping item; the key
  names the predicate directly.
...
- **N1.** No event-based triggers (watching for a specific keypress). The
  predicate is purely over observable session state, evaluated after any
  action.
- **N2.** No OR semantics within a trigger list in this spec.
- **N3.** No branching: a trigger always advances to the immediately
  following step, same as `space`.
- **N4.** No trigger on `space` or `Backspace` themselves — those are
  unconditional advance/retreat and cannot be predicated away.
...
A step may include a `trigger:` key whose value is a list of predicate
...
step's trigger — a chain of instantly-satisfied steps is legal and
...
- An `or` key present anywhere in a trigger list.
- An unknown key in a predicate item (any key other than `visible`,
  `folded`, `wire`, `type`, `caret`, `not`).
...
A trigger predicate whose position resolves to no node at evaluation time
is treated as **false** (the predicate is not satisfied). This is
...
`Step` gains a `trigger: Vec<Predicate>` field (empty by default, never
fires).
...
`RawStep` gains a `trigger: Option<Vec<RawPredicate>>` field;
`into_step` converts it to `Vec<Predicate>`. `RawPredicate` is an
untagged enum whose arms are recognized by which key is present:
...
```rust
fn script_trigger_satisfied(&self) -> bool
```

Returns true iff every predicate in the current step's trigger holds.
...
### Event-based triggers (watch for a specific keypress)
...
it), pressing `w` turns it *off*, and the trigger condition is still wire
bytes being visible. State-based triggers are correct in all cases;
key-based triggers require the script author to reason about entry state,
...
fully defines the starting state (spec 0271 S6), a trigger satisfied at
entry is unambiguously a script bug — the step's own directives satisfy
...
### OR semantics within a trigger list
...
1. `trigger_wire_advances_on_w` — a step with `wire: /1`; pressing `w`
   satisfies the predicate and the step index increments without `space`.
2. `trigger_not_satisfied_does_not_advance` — same setup; pressing a key
   that does not affect wire state leaves the step unchanged.
3. `trigger_fires_immediately_if_satisfied_at_entry` — a step whose
   `script_apply` already satisfies its own trigger skips to the next
   step on entry (G3).
4. `trigger_caret_predicate` — `caret: /2` fires when the cursor moves
   to `/2`, not before.
5. `trigger_type_predicate` — `type: /2 some.Type` fires after an
   override sets that type, not before.
6. `trigger_visible_and_folded_predicates` — `visible: /1` and
   `folded: /1` hold and fail at the right times; a leaf is never folded.
7. `trigger_all_predicates_must_hold` — a trigger with two items does not
   fire when only one holds.
8. `trigger_unresolvable_position_is_false` — a predicate whose position
   names no node evaluates to false; `space` still advances.
9. `trigger_not_inverts_predicate` — `[{not: [{wire: /1}]}]` fires when
   wire is off, not when it is on.
10. `trigger_not_of_conjunction` — `[{not: [{wire: /1}, {caret: /1}]}]`
    fires when at least one sub-predicate is false (De Morgan).
11. `trigger_not_is_recursive` — `not` inside `not` double-negates; the
    result is equivalent to the inner predicate.
12. `trigger_or_key_is_a_load_error` — a step with an `or` item in its
    trigger list fails at parse time with a message naming the key.
13. `trigger_unknown_key_is_a_load_error` — `dancing: /1` in a trigger
    list fails at parse time with a message naming the key.
14. `space_always_advances_regardless_of_trigger` — with a trigger not
    yet satisfied, `space` still advances the step (G2).
15. `reuse lint` passes.
